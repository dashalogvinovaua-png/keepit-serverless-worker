...
FILES = [
    ...
    # ПУТЬ ВНУТРИ РЕПОЗИТОРИЯ, А НЕ ГОЛОЕ ИМЯ. Стояло 'gemma4_e2b_it_bf16.safetensors', и
    # Hugging Face отвечал 404 Entry Not Found: файл лежит в подпапке text_encoders/. Проверено
    # чтением дерева репозитория 18.08.2026 — там семь файлов, и все в этой подпапке.
    (GEMMA, 'text_encoders/gemma4_e2b_it_bf16.safetensors', 'text_encoders', 10.28,
     'второй кодировщик'),
]
...
# РАЗМЕР ТОМА ПРИХОДИТ СНАРУЖИ, ЧИСЛОМ ОТ ЧЕЛОВЕКА ИЛИ ИЗ API RUNPOD.
# Изнутри контейнера квоту тома узнать НЕЛЬЗЯ, и это не мелочь, а причина сегодняшнего провала.
РАЗМЕР_ТОМА_ГБ = float(os.environ.get('VOLUME_GB') or 0)
КОРЕНЬ_ТОМА = os.environ.get('VOLUME_ROOT') or os.path.dirname(os.path.dirname(ROOT))


def занято(путь):
    """Сколько на томе лежит НА САМОМ ДЕЛЕ — обходом файлов, а не вопросом к df."""
    итого = 0
    for корень, _, файлы in os.walk(путь):
        for ф in файлы:
            try:
                итого += os.path.getsize(os.path.join(корень, ф))
            except OSError:
                pass
    return итого / 1e9
...
def свободно(путь):
    # ЗАЧЕМ ЗДЕСЬ НЕ df. 18.08.2026 этот скрипт отработал на живом томе и написал «свободно
    # 244919.8 ГБ», после чего КАЖДЫЙ файл упал с «Disk quota exceeded». Обе строки правдивы:
    # shutil.disk_usage читает общий кластер RunPod (643 ТБ, занят на 66%), а нам отведена
    # квота в полтораста гигабайт, и упираемся мы в неё. Прибор врал ровно в ту сторону, в
    # какую врать нельзя — «места полно», — и на этой лжи мы оплатили час дорогой карты.
    # Поэтому: свободное = размер тома (снаружи) минус занятое (обходом).
    os.makedirs(путь, exist_ok=True)
    if not РАЗМЕР_ТОМА_ГБ:
        return None
    return max(0.0, РАЗМЕР_ТОМА_ГБ - занято(КОРЕНЬ_ТОМА))
...
def главное():
    ...
    св = свободно(ROOT)
    if св is None:
        # НЕЗНАНИЕ МЕСТА — ЗАПРЕТ, А НЕ СКИДКА. Раньше здесь стояло число от df, и оно было
        # ложью; лучше честно отказаться, чем качать 49 ГБ в том, где их некуда положить.
        print('!! не знаю размера тома. Передайте его числом: VOLUME_GB=260 (смотреть в RunPod,')
        print('   GET /v1/networkvolumes). Из контейнера квота не читается — df показывает')
        print('   общий кластер и врёт на три порядка.')
        sys.exit(2)
    ...
    for repo, файл, куда, гб, зачем in FILES:
        ...
        try:
            ...
            # КВОТА ИСЧЕРПАНА — ДАЛЬШЕ ИДТИ НЕЗАЧЕМ. Раньше скрипт покорно пробовал все шесть
            # файлов и все шесть валил об одну стену, а в итоге писал шесть разных строк вместо
            # одного диагноза. Один внятный отказ полезнее шести одинаковых.
            if getattr(e, 'errno', None) == 122 or 'quota exceeded' in str(e).lower():
                print('\n!! КВОТА ТОМА ИСЧЕРПАНА — на томе нет места, и это не сетевая беда.')
                print('   Лечится расширением тома (PATCH /v1/networkvolumes/<id> {"size": ГБ})')
                print('   или сносом ненужного. Что сносить — решение владелицы, не скрипта.')
                break
